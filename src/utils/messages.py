...
def E_CONDITIONAL(fr, ft, tv):
    return f"O campo '{fr}' tornou-se obrigatório porque o campo '{ft}' foi informado com o valor '{tv}'."


def E_MAX_ITEMS(f, v, inst):
    return f"Quando o valor do campo '{f}' for '{v}', apenas ele deve constar na lista. Foir informado '{inst}'."
...
MULTIPLE_PATIENTS = "Foram encontrados múltiplos pacientes na busca. A seleção não pode ser realizada automaticamente."
...
def FIELD_VALUE_NOT_FOUND(field, value):
    return f"Valor '{value}' não encontrado na lista de opções válidas para o campo '{field}'."


def INVALID_FIELD_VALUE_OPTIONS(field, value, opts):
    return f"O valor '{value}' fornecido para o campo '{field}' não consta na lista de opções válidas. Opções válidas: {', '.join(opts)}."
...
ANO_MAMOGRAFIA_REQUIRED = "Campos 'Ano' de 'QUANDO FEZ A ÚLTIMA MAMOGRAFIA?' do card 'FEZ MAMOGRAFIA ALGUMA VEZ?' é obrigatório."
ANO_RADIOTERAPIA_REQUIRED = (
    "Campos de ano da radioterapia obrigatórios conforme a localização."
)
